...
# get topic by month
def get_topic_by_month(topic_url):
    ...
    topic_class = driver.find_element_by_class_name("user-right")
    div = topic_class.find_elements_by_tag_name("div")[0].find_elements_by_tag_name("div")[0].find_elements_by_tag_name("div")

    # each topic has one time; get all time, appending to a list
    topic_time_list = []

    # if fail, get it again
    if len(div) <= 0:
        count = 3
        while (count > 0):
            print("Fail to get topics, trying again ... please wait " + str(3 - count + 1) + " times !")
            driver.close()
            webdriver.PhantomJS()
            driver.get(topic_url)

            topic_class = driver.find_element_by_class_name("user-right")
            div = topic_class.find_elements_by_tag_name("div")[0].find_elements_by_tag_name("div")[0] \
    ...
    # get topic by month
    length = transfer_topic_time(topic_time_list)
...
# get reply by month
def get_reply_by_month(reply_url):
    ...
    reply_class = driver.find_element_by_class_name("user-right")
    reply_time_list = reply_class.find_elements_by_tag_name("div")[0].find_elements_by_class_name("time")

    # if fail, get it again
    if len(reply_time_list) < 0:
        count = 3
        while(count > 0):
            print("Fail to get replies, trying again ... please wait " + str(3 - count + 1) + " times !")
            driver.close()
            webdriver.PhantomJS()
            driver.get(topic_url)
            reply_class = driver.find_element_by_class_name("user-right")
...
def get_topic_reply(topic_reply_monthly_url):
...
def get_solution_by_month(solution_url):
    ...
    # get solution by month
    length = transfer_stamp_time(solved_stamp_list)
...
if __name__ == '__main__':
    ...
    for i in range(len(PDLENCRDVT_MEMBERS_NAMES_AND_EIDS)):
        ...
        if user_defined == "Y" or user_defined == "y":
            get_topic_by_month(topic_url)
            get_reply_by_month(reply_url)
            get_solution_by_month(solved_url)
        else:
            # use discourse provided tool to query topic and reply
            get_topic_reply(topic_reply_monthly_url)
