...
# s = ("Hi, Girl!").encode("utf-8")
# pygame.display.set_caption(str(s, encoding="utf-8"))

# pygame.display.set_caption((u'小姐姐 你的快递到了。').encode('utf-8').decode("utf-8"))
# pygame.display.set_caption((u"基础 pygame 程序").encode('utf-8'))
pygame.display.set_caption((u"你好").encode("UTF-8").decode("UTF-8"))

# 添加文本信息
def title(text, screen, scale, color=(0, 0, 0)):
    font = pygame.font.SysFont('SimHei', 27)
    # 或者采用下面的方式指定系统自带的字体
    # font = pygame.font.Font('C:\Windows\Fonts\simfang.ttf', 27)
...
def show_like_interface(screen):
    ...
    background1 = pygame.image.load("C:\\MySoftWare\\Pycharm\\PycharmTest\\love\\214_1.jpg").convert()
...
def main():
    ...
    while running:
        ...
        screen.fill((255, 255, 255))
        # 添加背景图
        background = pygame.image.load('C:\MySoftWare\Pycharm\PycharmTest\love\\214_2.jpg').convert()
        screen.blit(background, (0, 0))

        # 获取鼠标位置
        pos = pygame.mouse.get_pos()
        # 判断鼠标位置,不同意时,按钮不断变化
        if pos[0] < unlike_pos_x + unlike_pos_width + 5 and \
            pos[0] > unlike_pos_x -5 and \
            pos[1] < unlike_pos_y + unlike_pos_height + 5 \
            and pos[1] > unlike_pos_y -5:
            while True:
        ...
        # 设置标题及其按钮文本信息
        title('1.如果有一天我想你表白, 你会怎么样?', screen, scale=[8, 3])
        ...
        # 当鼠标点击同意按钮，就跳转结束页面
        if pos[0] < like_pos_x + like_pos_width + 5 and \
            pos[0] > like_pos_x -5 and \
            pos[1] < like_pos_y + like_pos_height + 5 \
            and pos[1] > like_pos_y -5:
