...
def get_topic_reply(topic_reply_monthly_url, EID):
    ...
    if number > 0:
        ...
        print("Fail to get topics and replies, trying again ... please wait !")
        browser.get(topic_reply_monthly_url)
        topic_reply_list = browser.find_elements_by_class_name("number")
        topic_number = topic_reply_list[2].text
        reply_number = topic_reply_list[3].text
...
def transfer_time(time_list):
    ...
    for tm in time_list:
        ...
        # 1. 3 days   /5 hours
        if days in tm.text or hours in tm.text:
        ...
        # 3. Dec 17, '18
            ...
            if (int(str(year)[-2:]) - int(str(tm.text).split("'")[1]) == 1) \
...
def plot_score(score_list):
    ...
    if month == 1:
        plt.xlabel("Period: from " + str(year - 1) + "_" + str(12) + "_" + str(day) + " to " + str(year) + "_" + str(
            month) + "_" + str(day))
    else:
        plt.xlabel("Period: from " + str(year) + "_" + str(month - 1) + "_" + str(day) + " to " + str(year) + "_" + str(
            month) + "_" + str(day))
    plt.ylabel('score')
