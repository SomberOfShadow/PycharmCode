...
def plot_score(score_list):
    ...
    # X/Y 轴标题,X轴是期间,但是要注意的是月份是1的时候要做特殊的判断
    if month == 1:
    ...
    # plt.savefig(pic_name) # 此种方式保存到额图片不完整
...
# path = "C:\\MySoftWare\\Pycharm\\PycharmTest\\discourse\\score_" + str(year)+ "_" + str(month) + "_" + str(day) + ".txt"
