...
class FindLocation (object):
    def __init__(self):
        # GeoLiteCity.dat是从csdn上下载下来的
        self.gi = pygeoip.GeoIP("C:\MySoftWare\Pycharm\PycharmTest\gui_demo\\GeoLiteCity.dat")
        # 创建主窗口 用UI容纳其他的组件
    ...
    def find_position(self):
        ...
        # 为了避免非法制，导致程序崩溃，有心去的可以用正则写一下具体的规则
        try:
            # 获取目标城市
            city = aim["city"]
            # 获取目标国家
        ...
        the_ip_info = ["所在纬度：" + str(latitude), "所在经度：" + str(longitude), "地域代号：" + str(region_code),
                        "所在城市：" + str(city), "所在国家和地区：" + str(country), "需要查询的ip：" + str(self.ip_addr)]
        # 清空回显列表可见部分， 类似于clear命令
